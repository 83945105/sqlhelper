...
import com.dt.core.exception.SqlException;
...
public abstract class AbstractMySqlBuilder<M extends Model> extends AbstractSqlBuilder {
    ...
    private SqlSplicer appendFunctionColumnSql(SqlSplicer sqlSplicer) {
        ...
        for (FunctionColumnData fcData : this.sqlData.getFunctionColumnDataList()) {
            ...
            String alias;
            for (Map.Entry<String, String> entry : fcData.getColumnAliasMap().entrySet()) {
    ...
    protected SqlSplicer appendColumnSql(SqlSplicer sqlSplicer) {
        ...
        if (hasF) {
            sqlSplicer = this.appendFunctionColumnSql(sqlSplicer.append(","));
        }
        if (hasV) {
            if (hasF) {
                sqlSplicer.append(",");
        ...
        if (hasC) {
            ...
            sqlSplicer = this.appendTableColumnSql(sqlSplicer.append(","));
        }
        return sqlSplicer;
    }
    ...
    private SqlSplicer appendLinkWhereDataList(SqlSplicer sqlSplicer, List<LinkWhereData> linkWhereDataList, LinkType linkType) {
        if (linkWhereDataList == null || linkWhereDataList.size() == 0) {
            return sqlSplicer;
        }
        if (linkType == LinkType.OR && linkWhereDataList.size() > 1) {
            sqlSplicer.append("(");
        }
            ...
            if (whereDataList != null && whereDataList.size() > 0) {
                ...
                switch (linkWhereData.getLinkType()) {
                    case AND:
                        ...
                        this.appendLinkWhereDataList(sqlSplicer, childLinkWhereDataList, LinkType.AND);
                        continue;
                    case OR:
                        if (i++ > 0) {
                            sqlSplicer.append(" or ");
                        }
                        this.appendLinkWhereDataList(sqlSplicer, childLinkWhereDataList, LinkType.OR);
                        continue;
                    default:
                        throw new SqlException("the LinkType is wrong.");
                }
    ...
    protected SqlSplicer appendLimitSql(SqlSplicer sqlSplicer) {
        Pagination pagination = this.sqlData.getPagination();
        if (pagination == null) {
            return sqlSplicer;
        }
