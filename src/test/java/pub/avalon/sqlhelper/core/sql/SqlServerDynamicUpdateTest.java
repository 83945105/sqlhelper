...
/**
 * SqlServer动态引擎 - 更新 - 条件更新
 */
public class SqlServerDynamicUpdateTest extends AbstractTest {
    ...
    void TestUpdateJavaBean() {
        ...
        SqlBuilder sqlBuilder = SqlServerDynamicEngine.update(SysUserModel.class)
                .column(table -> table)
                .updateJavaBean(javaBean);
        setSqlBuilder(sqlBuilder, "update SysUser set SysUser.[id] = ?,SysUser.[user_name] = ?,SysUser.[login_name] = ? from [sys_user] SysUser");
    }
    ...
    void TestUpdateJavaBeanAndJoinAndWhere() {
        ...
        SqlBuilder sqlBuilder = SqlServerDynamicEngine.update(SysUserModel.class)
                ...
                .updateJavaBean(javaBean);
        setSqlBuilder(sqlBuilder, "update SysUser set SysUser.[user_name] = ? from [sys_user] SysUser inner join [user_role] UserRole on UserRole.[role_id] = SysUser.[id] where UserRole.[role_name] = ?");
    }
    ...
    void TestUpdateOrInsertJavaBeans() {
        ...
        sysUser.setUserName(arg());
        javaBeans.add(sysUser);
        sqlBuilder = SqlServerDynamicEngine.update(SysUserModel.class)
